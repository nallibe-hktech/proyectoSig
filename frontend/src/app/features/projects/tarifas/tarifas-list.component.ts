...
export class TarifasListComponent implements OnInit {
  ...
  protected readonly tarifas = signal<TarifaProyectoDto[]>([]);
  ...
  private load(): void {
    ...
    this.tarifasSvc.list(this.projectId).subscribe({
      next: (data) => {
  ...
  protected onEdit(tarifa: TarifaProyectoDto): void {
    this.dialog.open(TarifaFormComponent, {
      data: { projectId: this.projectId, tarifa, mode: 'edit' },
      minWidth: 500,
    }).afterClosed().subscribe((ok) => {
      if (ok) this.load();
    });
  ...
  protected onDelete(tarifa: TarifaProyectoDto): void {
    this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar Tarifa', message: 'Acción irreversible.', entityName: tarifa.nombre, destructive: true },
      minWidth: 480,
    }).afterClosed().subscribe((ok) => {
      if (!ok) return;
      this.tarifasSvc.delete(tarifa.id, this.projectId).subscribe({
        next: () => {
