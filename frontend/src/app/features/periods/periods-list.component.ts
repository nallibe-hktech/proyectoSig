...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DatePipe, RouterLink, MatCardModule, MatTableModule, MatButtonModule, MatIconModule, MatChipsModule, MatDialogModule, BreadcrumbsComponent, SkeletonComponent, EmptyStateComponent],
  template: `
    <div class="sig-page">
...
export class PeriodsListComponent implements OnInit {
  ...
  protected onCerrar(row: PeriodDto): void {
    this.dialog.open(ConfirmDialogComponent, {
      ...
      this.periodSvc.cerrar(row.id).subscribe({
        next: () => { this.notify.success('Período cerrado'); this.load(); },
  ...
  protected onReabrir(row: PeriodDto): void {
    this.dialog.open(ConfirmDialogComponent, {
      ...
      this.periodSvc.reabrir(row.id).subscribe({
        next: () => { this.notify.success('Período reabierto'); this.load(); },
  ...
  private load(): void {
    ...
    this.periodSvc.list().subscribe({
      next: (ps) => { this.items.set(ps); this.loading.set(false); },
