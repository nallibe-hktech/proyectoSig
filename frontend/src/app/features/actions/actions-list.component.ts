...
@Component({
  ...
  template: `
    <div class="sig-page">
      <sig-breadcrumbs [crumbs]="[{ label: 'Inicio', route: '/dashboard' }, { label: 'Actions' }]" />
      <div class="sig-page__header">
        <h1 class="sig-page__title">Actions</h1>
        <a mat-flat-button color="primary" routerLink="/actions/nuevo" data-testid="btn-nuevo"><mat-icon>add</mat-icon> Nueva Action</a>
      </div>
      <mat-card>
        <mat-card-content>
          ...
          @else if (items().length === 0) {
            <sig-empty-state icon="task_alt" title="No hay actions todavía" ctaLabel="Crear primera action" [hasFilter]="!!search.value" (ctaClick)="onEmptyCta()" />
          } @else {
            <table mat-table [dataSource]="items()" class="sig-table" data-testid="tabla-actions">
              ...
              <ng-container matColumnDef="estado"><th mat-header-cell *matHeaderCellDef>Estado</th><td mat-cell *matCellDef="let row"><mat-chip>{{ row.estado }}</mat-chip></td></ng-container>
...
export class ActionsListComponent implements OnInit {
  ...
  protected readonly cols = ['nombre', 'proyecto', 'estado', 'acciones'];
  ...
  protected onDelete(row: ActionListItemDto): void {
    this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar Action', message: 'Acción irreversible.', entityName: row.nombre, destructive: true }, minWidth: 480,
    }).afterClosed().subscribe((ok) => {
      if (!ok) return;
      this.actionSvc.delete(row.id).subscribe({ next: () => { this.notify.success('Action eliminada'); this.load(); }, error: (err) => this.notify.error(err?.error?.title ?? 'No se pudo eliminar') });
    });
  }
  ...
  private load(): void {
    ...
    this.actionSvc.list(this.page(), this.pageSize(), null, this.search.value).subscribe({
