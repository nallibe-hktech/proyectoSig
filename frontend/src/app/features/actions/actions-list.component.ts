    ...
    <div class="sig-list-page">
      <div class="sig-list-topbar">
        ...
        <div class="sig-topbar-actions">
          <button class="sig-btn-outline" (click)="exportCsv()"><mat-icon>download</mat-icon> Exportar CSV</button>
          <button class="sig-btn-primary" (click)="openNew()"><mat-icon>add</mat-icon> Nueva Accion</button>
        </div>
      </div>
            ...
            <tbody>
              @for (a of filtered(); track a.id) {
                <tr (click)="selectRow(a)" [class.selected]="selected()?.id === a.id">
                  ...
                  <td><span class="sig-badge sig-badge--green">Activa</span></td>
                  <td style="font-size:11px;color:var(--sig-text-muted)">{{ '035501' }}</td>
                  <td>
                    <div class="sig-row-actions">
                      <button class="sig-icon-btn" title="Ver" (click)="$event.stopPropagation(); selectRow(a)"><mat-icon>visibility</mat-icon></button>
                ...
                <tr><td colspan="6"><div class="sig-empty"><mat-icon>bolt</mat-icon><span class="sig-empty-title">Sin acciones</span></div></td></tr>
              }
            </tbody>
          </table>
        ...
        @if (selected()) {
          <div class="sig-detail-panel">
            ...
            <div class="sig-detail-body">
              <div class="sig-detail-grid">
                ...
                <div class="sig-detail-field">
                  ...
                  <span>{{ selected()!.projectNombre }}</span>
                </div>
                <div class="sig-detail-field">
                  <label>CECO</label>
                  <span>{{ '035501' }}</span>
                ...
                <div class="sig-detail-field">
                  ...
                  <span>Silvia Garzon</span>
                </div>
                <div class="sig-detail-field">
                  <label>Origen</label>
                  <span>Celero One</span>
                </div>
              </div>

              <div class="sig-detail-section">
                <div class="sig-detail-section-title">
                  Conceptos Asociados <span>2</span>
                </div>
                <div class="sig-concept-row">
                  <span class="col-id">143</span>
                  <div><div style="font-size:12px;font-weight:500">Nota de gastos pago</div><div style="font-size:10px;color:var(--sig-text-muted)">01/01/2023</div></div>
                  <span class="sig-badge sig-badge--yellow" style="font-size:10px;padding:1px 6px">Pago</span>
                  <span style="font-size:11px;color:var(--sig-text-muted)">Suma de Gasto</span>
                </div>
                <div class="sig-concept-row">
                  <span class="col-id">78</span>
                  <div><div style="font-size:12px;font-weight:500">Nota de gastos facturacion</div><div style="font-size:10px;color:var(--sig-text-muted)">12/07/2019</div></div>
                  <span class="sig-badge sig-badge--blue" style="font-size:10px;padding:1px 6px">Factura</span>
                  <span style="font-size:11px;color:var(--sig-text-muted)">Suma de Gasto</span>
                </div>
                <span class="sig-add-link">+ Añadir Concepto &nbsp;&middot;&nbsp; + Nuevo Concepto</span>
              </div>

              <div class="sig-detail-section">
                <div class="sig-detail-section-title">Usuarios <span>2</span></div>
                <div class="sig-avatar-chip">
                  <div class="sig-avatar" style="background:#0d9488">TM</div>
                  <div class="sig-avatar-info">
                    <span class="sig-avatar-name">Tomas Martin</span>
                    <span class="sig-avatar-role">Interlocutor</span>
                  </div>
                  <button class="sig-quitar-btn">Quitar</button>
                </div>
                <div class="sig-avatar-chip">
                  <div class="sig-avatar" style="background:#7c3aed">JM</div>
                  <div class="sig-avatar-info">
                    <span class="sig-avatar-name">Jesus Mata</span>
                    <span class="sig-avatar-role">Gestor</span>
                  </div>
                  <button class="sig-quitar-btn">Quitar</button>
                </div>
            ...
            <div class="sig-detail-footer">
              ...
              <button class="sig-btn-dup"><mat-icon>content_copy</mat-icon> Duplicar</button>
...
`,
  styles: [`
    ...
    .sig-btn-filter { height: 36px; padding: 0 16px; border-radius: 8px; background: var(--sig-blue); color: #fff; border: none; font-size: 13px; font-weight: 600; font-family: inherit; cursor: pointer; &:hover { background: var(--sig-blue-light); } }
    ...
    .sig-btn-dup { flex: 1; height: 34px; border-radius: 8px; border: 1px solid var(--sig-border); background: transparent; color: var(--sig-text-primary); font-size: 13px; font-family: inherit; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; mat-icon { font-size: 15px !important; width: 15px !important; height: 15px !important; } }
    ...
    .sig-concept-row { display: grid; grid-template-columns: 40px 1fr 60px 80px; gap: 8px; align-items: center; padding: 6px 0; border-bottom: 1px solid var(--sig-border); font-size: 12px; &:last-child { border-bottom: none; } }
    .sig-avatar-chip { display: flex; align-items: center; gap: 8px; padding: 6px 0; }
    ...
    .sig-add-link { font-size: 12px; color: var(--sig-blue); cursor: pointer; margin-top: 6px; display: inline-flex; align-items: center; gap: 4px; }
    ...
    .sig-page-btn { width: 28px; height: 28px; border-radius: 6px; border: 1px solid var(--sig-border); background: transparent; color: var(--sig-text-muted); cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; &:hover:not(:disabled) { background: var(--sig-bg-hover); } &:disabled { opacity: .35; cursor: not-allowed; } }
