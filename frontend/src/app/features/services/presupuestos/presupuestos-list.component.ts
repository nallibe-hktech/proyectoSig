...
export class PresupuestosListComponent implements OnInit {
  ...
  // Permite uso como ruta /services/:id/presupuestos (withComponentInputBinding bindea :id -> id)
  ...
  protected readonly presupuestos = signal<PresupuestoServicioDto[]>([]);
  ...
  private load(): void {
    ...
    this.presupuestosSvc.list(this.serviceId).subscribe({
      next: (data) => {
  ...
  protected onEdit(presupuesto: PresupuestoServicioDto): void {
    this.dialog.open(PresupuestoFormComponent, {
      data: { serviceId: this.serviceId, presupuesto, mode: 'edit' },
      minWidth: 500,
    }).afterClosed().subscribe((ok) => {
      if (ok) this.load();
    });
  ...
  protected onDelete(presupuesto: PresupuestoServicioDto): void {
    ...
    this.dialog.open(ConfirmDialogComponent, {
      ...
      if (!ok) return;
      this.presupuestosSvc.delete(presupuesto.id, this.serviceId).subscribe({
        next: () => {
